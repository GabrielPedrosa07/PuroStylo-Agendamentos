...
if (empty($id_agd) || empty($cliente_id) || empty($servico_id)) {
    echo 'Dados insuficientes para processar.';
    exit();
}
...
try {
    ...
    if (!$servico_dados) {
        throw new Exception("Serviço com ID '$servico_id' não foi encontrado!");
    }

    $valor_base_servico = $servico_dados['valor'];
    $comissao_config = $servico_dados['comissao'];
    ...
    if ($tipo_comissao == 'Porcentagem') {
        $valor_comissao = ($comissao_config * $valor_base_servico) / 100;
    } else {
        $valor_comissao = $comissao_config;
    }
    ...
    $pdo->rollBack();
    error_log("Erro ao finalizar serviço: " . $e->getMessage()); // Loga o erro para o admin
    echo 'Falha ao salvar no banco de dados. Por favor, tente novamente.';
} catch (Exception $e) {
    // Captura outras exceções (como "Serviço não encontrado")
    $pdo->rollBack();
