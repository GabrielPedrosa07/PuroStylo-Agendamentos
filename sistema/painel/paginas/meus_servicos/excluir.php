...
// Certifique-se de que o ID foi enviado e não está vazio
if (empty($_POST['id'])) {
...
try {
    ...
    $query_select = $pdo->prepare("SELECT * FROM $tabela WHERE id = :id");
    $query_select->bindValue(":id", $id);
    $query_select->execute();
    $dados_receber = $query_select->fetch(PDO::FETCH_ASSOC); // Usar fetch() pois esperamos apenas 1 resultado

    // 2. VERIFICA SE O REGISTRO REALMENTE EXISTE
    // BOA PRÁTICA: Sempre verificar se a consulta retornou resultados antes de usá-los.
    if ($dados_receber) {
        $foto = $dados_receber['foto'];
        $produto_id = $dados_receber['produto'];
        $quantidade = $dados_receber['quantidade'];

        // 3. APAGA A FOTO ASSOCIADA (se houver)
        if ($foto != "sem-foto.jpg" && file_exists('../../img/contas/'.$foto)) {
            @unlink('../../img/contas/'.$foto);
        }

        // 4. DEVOLVE A QUANTIDADE AO ESTOQUE DO PRODUTO
        ...
        // 5. FINALMENTE, EXCLUI O REGISTRO DA TABELA 'receber'
    ...
    // BOA PRÁTICA: Capturar exceções do banco de dados para tratar erros.
